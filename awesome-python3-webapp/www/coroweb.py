...
def add_route(app, fn):
    ...
    if path is None or method is None:
        raise ValueError('@get or @post noe defined in %s.' % str(fn))
...
def add_routes(app, modult_name):
    n = modult_name.rfind('.')
    if n == (-1):
        mod = __import__(modult_name, globals(), locals())
    else:
        name = modult_name[n+1:]
        mod = getattr(__import__(modult_name[:n], globals(), locals(), [name]), name)
    for attr in dir(mod):
